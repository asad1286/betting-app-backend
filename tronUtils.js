...
async function sendTRX(user, fromAddress, toAddress, amount) {
  try {
    ...
    console.log(`Sending ${amount} TRX to ${toAddress} on mainnet`);
...
async function sendTRXFromUserOnWithdraw(user, fromAddress, toAddress, amount) {
  try {
    if (!user.trx20PrivateKey) throw new Error("User private key is missing");
    if (!tronWeb.isAddress(toAddress)) throw new Error("Invalid recipient TRX address");
    const userTronWeb = new TronWeb({
...
async function sendWithDrawAmount(toAddress, amount,userAddress,userPrivateKey) {
  try {
    const tronWeb = new TronWeb({
      fullHost: 'https://api.trongrid.io',
    ...
    const transaction = await tronWeb.trx.sendTransaction(toAddress, amountInSun, { from: userAddress });
    console.log(`Transaction successful: ${transaction?.txid}`);
    return transaction.txid;
  } catch (error) {
...
async function fetchTRXTransactions(ADMIN_ADDRESS) {
  try {
    const url = `https://api.trongrid.io/v1/accounts/${ADMIN_ADDRESS}/transactions?limit=200`;
